...
class Document_Model extends CI_Model
{
  ...
  function get_document($params = array())
  {
    $result = '';
    if (!empty($params['search_for'])) {
      $this->db->select("count(ft.document_id) as counts");
    } else {
    ...
    $this->db->from("document as ft");
    $this->db->order_by("document_id desc");

    if (!empty($params['document_id'])) {
      $this->db->where("ft.document_id", $params['document_id']);
    }
    ...
    if (!empty($params['record_status'])) {
      if ($params['record_status'] == 'zero') {
        ...
        $this->db->where("ft.document_id", $params['record_status']);
      }
    }
